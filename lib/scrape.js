...
export const scrapePlayerData = async (url) => {
  ...
  try {
    ...
    const playerItemIds = data.players.map((player) => player.playerItemId)

    const allPlayersFullData = []

    for (const playerId of playerItemIds) {
      await page.evaluate(async (playerId) => {
        const playerStatsItemSelector = `div[data-playerid="${Number(playerId)}"]`
        const playerStatsElement = document.querySelector(playerStatsItemSelector)
      ...
      const evaluateData = { data, playerId }

      const playerFullData = await page.evaluate(async (evaluateData) => {
        const { data, playerId } = evaluateData
        ...
        if (subStatRatingDivs.length === 0) return null
